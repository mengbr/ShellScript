# XI SEMANA DE ENGENHARIA E TECNOLOGIA - Engenharia de Software
#  TodoList em Shell Script
- Ferramenta para gerenciar uma lista de tarefas em shell. O código foi gerado em sala de aula para explicar os conceitos de shell script em bash. 
- Local: Universidade de Ribeirão Preto
- Professor: Michael Oliveira - [Linkedin](https://www.linkedin.com/in/michaelengbr/)
- Data: 28/04/2026 a 30/04/2026
- [Slides MiniCurso](SlideMiniCursoShell.pdf)

## Referencias para consulta e aperfeiçoamento
- Canivete shell: https://aurelio.net/shell/canivete/
- Curso Shell Script profissional: https://www.youtube.com/@Geofisicando/
- Livro Shell Script Profissional: https://www.novatec.com.br/livros/shellscript/
